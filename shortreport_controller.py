...
def fetch_player_details(player_id):
    try:
        ...
        player_details = {
            ...
            'player_club': player.teams[-1].team_id.name,  # Assuming the last team is the current team
...
def fetch_match_details(match_id, player_team_id):
    try:
        ...
        opposition_club = match.away_team.name if match.home_team.id == return_oid(player_team_id) else match.home_team.name
